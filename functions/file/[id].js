        ...
        try {
            if (!metadata.Label) {
                ...
                if (moderateResponse.ok) {
                    ...
                    console.log('[onRequest] ModerateContent data =', JSON.stringify(moderateData));
                    if (moderateData && moderateData.rating_label) {
                        metadata.Label = moderateData.rating_label;
                        console.log(`[onRequest] rating_label = ${metadata.Label}`);
                        if (moderateData.rating_label === "adult") {
                            await env.img_url.put(filenameToUse, "", { metadata });
                            console.log('[onRequest] Content labelled adult. Redirecting to block-img.html');
                            return Response.redirect(`${url.origin}/block-img.html`, 302);
                        }
                    }
                } else {
                    console.error('[onRequest] ModerateContent API fetch not OK');
                }
            }
        } catch (error) {
            console.error("[onRequest] Error during content moderation:", error.message);
        }
    } else {
        console.log('[onRequest] ModerateContentApiKey is not set; skipping moderation.');
    }

    try {
        await env.img_url.put(filenameToUse, "", { metadata });
        console.log('[onRequest] Updated metadata in KV store.');
    } catch (e) {
        console.error('[onRequest] Error updating metadata in KV:', e.message);
    }
